    ...
    async edit(req, res) {
        try {
            ...
            if (!_.isEmpty(settingValue)) {
                ...
                if (settingValue.setting_slug == 'welcome_notification_message') {
                    result.setting_data.setting_value = unescape(result.setting_data.setting_value);
                }
    ...
    */
    async update(req, res) {
        try {
            ...
            if (req.body.slug == 'welcome_notification_message') {
                req.body.setting_value = escape(unescape(req.body.setting_value));
            }
    ...
    */
    async getAllData(req, res) {
        ...
        try {
            ...
            for (var i = 0; i < settingValue.data.length; i++) {
                if (settingValue.data[i].setting_slug == 'welcome_notification_message') {
                    var html = unescape(settingValue.data[i].setting_value);
                    var data_sort = striptags(html, '').substring(0, 25);
                    Object.assign(settingValue.data[i], { "setting_value": data_sort });
                }
            }
